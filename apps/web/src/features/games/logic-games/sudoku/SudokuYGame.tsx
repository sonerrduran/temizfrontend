...
const generateSolvedBoard = (size: number): number[][] => {
  ...
  // Y Shape constraint: upper half diagonals meeting in middle, straight down
  const isYShape = (r: number, c: number) => {
    ...
    if (r < midRow) {
      ...
      return c === (size % 2 === 0 ? midCol - 1 : midCol);
    }
  };
  ...
  const isValid = (r: number, c: number, num: number) => {
    ...
    // Y shape constraint check
    if (isYShape(r, c)) {
      for (let tr = 0; tr < size; tr++) {
        for (let tc = 0; tc < size; tc++) {
          if (isYShape(tr, tc) && board[tr][tc] === num) return false;
        }
      }
    }
    return true;
...
const SudokuYGame: React.FC<SudokuYGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-yellow-500/30 shadow-2xl relative overflow-hidden bounce-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-center flex-1">
          <h2 className="text-2xl md:text-3xl font-black italic text-yellow-400 drop-shadow-lg uppercase tracking-tight">
            Sudoku Y
          </h2>
          <div className="text-xs font-bold text-yellow-200 mt-1 uppercase tracking-widest">
            {size}x{size} Izgara | (Temel Sürüm) | Hata: {mistakes}/{maxMistakes}
          </div>
        </div>
        <div
          className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-yellow-400 text-yellow-400'}`}
        >
          {formatTime(timeLeft)}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-yellow-950/50 p-2 md:p-3 rounded-xl border-4 border-yellow-900/50 shadow-[0_0_20px_rgba(234,179,8,0.1)] grid gap-0.5 mb-8 relative"
          style={{
            ...
            row.map((cell, c) => {
              ...
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1
                  ? 'border-r-4 border-yellow-700/80'
                  : 'border-r border-yellow-900/50';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1
                  ? 'border-b-4 border-yellow-700/80'
                  : 'border-b border-yellow-900/50';

              const shapeCell = isYShape(r, c);
              ...
              return (
                <button
                  ...
                  className={`
                     ...
                     ${isFixed ? (shapeCell ? 'bg-yellow-900/70 text-yellow-100' : 'bg-yellow-900/40 text-yellow-100') : shapeCell ? 'bg-yellow-800/40 text-yellow-300' : 'bg-slate-800/30 text-yellow-300'} hover:bg-slate-800/50
                     ${isSelected ? 'ring-inset ring-4 ring-yellow-400 bg-yellow-600/50 shadow-[0_0_15px_rgba(234,179,8,0.5)] z-20' : ''}
                   `}
                >
                  {cell !== 0 ? cell : ''}
                </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-yellow-500 to-amber-700 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-yellow-400/50"
            >
              {num}
            </button>
          ))}
