...
interface ArrowSudokuGameProps {
  grade: number;
...
const ArrowSudokuGame: React.FC<ArrowSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-orange-500/30 shadow-2xl relative overflow-hidden bounce-in">
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-orange-950/50 p-2 md:p-3 rounded-xl border-4 border-orange-800/50 shadow-inner grid gap-0.5 mb-8 relative"
          style={{
            gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))`,
            width: size === 4 ? '280px' : '360px',
          }}
        >
          {/* Decorative Arrows Overlay */}
          <div className="absolute inset-0 pointer-events-none opacity-20 flex items-center justify-center">
            <span className="text-6xl transform rotate-45 text-orange-300">↗</span>
            <span className="text-6xl transform -rotate-45 text-orange-300 ml-8">↘</span>
          </div>
            ...
            row.map((cell, c) => {
              ...
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1
                  ? 'border-r-4 border-orange-700/80'
                  : 'border-r border-orange-900/50';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1
                  ? 'border-b-4 border-orange-700/80'
              ...
              return (
                <button
                  ...
                  className={`
                     ...
                     ${isFixed ? 'bg-orange-900/40 text-orange-100 cursor-default' : 'bg-slate-800/30 text-orange-300 hover:bg-slate-800/50'}
                     ${isSelected ? 'ring-inset ring-4 ring-orange-400 bg-orange-600/50 shadow-[0_0_15px_rgba(249,115,22,0.5)] z-10' : ''}
                   `}
                >
                  {cell !== 0 ? cell : ''}
                </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-orange-500 to-amber-700 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-orange-400/50"
            >
              {num}
            </button>
          ))}
...
export default ArrowSudokuGame;
