...
interface EvenOddSudokuGameProps {
  grade: number;
...
const EvenOddSudokuGame: React.FC<EvenOddSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-fuchsia-500/30 shadow-2xl relative overflow-hidden bounce-in">
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-purple-950/50 p-2 md:p-3 rounded-xl border-4 border-fuchsia-900/50 shadow-inner grid gap-1 mb-8"
          style={{
        ...
        >
          {board.map((row, r) =>
            row.map((cell, c) => {
              ...
              const targetVal = solvedBoard[r][c];
              const isOdd = targetVal % 2 !== 0;

              const boxRows = size === 4 ? 2 : 2;
              const boxCols = size === 4 ? 2 : 3;
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-4 border-fuchsia-700/80' : '';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-4 border-fuchsia-700/80' : '';

              // Only show odd/even hint via shape/bg for empty cells or fixed cells
              const cellBg = isOdd ? 'bg-indigo-900/40 rounded-full' : 'bg-pink-900/40 rounded-lg';
              ...
              return (
                <button
                  ...
                  className={`
                     ...
                     ${cellBg}
                     ${isFixed ? 'text-fuchsia-100 cursor-default' : 'text-fuchsia-300 hover:bg-fuchsia-800/50'}
                     ${isSelected ? 'ring-inset ring-4 ring-fuchsia-400 bg-fuchsia-600/50 shadow-[0_0_15px_rgba(217,70,239,0.5)] z-10' : ''}
                   `}
                >
                  {cell !== 0 ? cell : ''}
                </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className={`w-14 h-14 md:w-16 md:h-16 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-fuchsia-400/50 ${num % 2 !== 0 ? 'bg-gradient-to-br from-indigo-500 to-indigo-700' : 'bg-gradient-to-br from-pink-500 to-pink-700'}`}
            >
              {num}
            </button>
          ))}
        ...
        <div className="mt-4 flex gap-4 text-xs font-bold text-white/50">
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-indigo-500"></div> Tek Sayı
          </span>
          <span className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-sm bg-pink-500"></div> Çift Sayı
          </span>
        </div>
      </div>
...
export default EvenOddSudokuGame;
