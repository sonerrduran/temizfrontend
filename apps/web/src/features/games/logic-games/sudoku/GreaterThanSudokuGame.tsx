...
interface GreaterThanSudokuGameProps {
  grade: number;
...
const GreaterThanSudokuGame: React.FC<GreaterThanSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-green-500/30 shadow-2xl relative overflow-hidden bounce-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-center flex-1">
          <h2 className="text-2xl md:text-3xl font-black italic text-green-400 drop-shadow-lg uppercase tracking-tight">
            {'>'} Büyük/Küçük Sudoku
          </h2>
          <div className="text-xs font-bold text-green-200 mt-1 uppercase tracking-widest">
            {size}x{size} Izgara | (Temel Sürüm) | Hata: {mistakes}/{maxMistakes}
          </div>
        </div>
        <div
          className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-green-400 text-green-400'}`}
        >
          {formatTime(timeLeft)}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-green-950/50 p-2 md:p-3 rounded-xl border-4 border-green-900/50 shadow-[0_0_20px_rgba(34,197,94,0.1)] grid gap-0.5 mb-8 relative"
          style={{
            ...
            row.map((cell, c) => {
              ...
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1
                  ? 'border-r-4 border-green-700/80'
                  : 'border-r border-green-900/50';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1
                  ? 'border-b-4 border-green-700/80'
                  : 'border-b border-green-900/50';

              // Determine > / < randomly for decorative purposes
              const RightRelation =
                c < size - 1 ? (solvedBoard[r][c] > solvedBoard[r][c + 1] ? '>' : '<') : null;
              const BottomRelation =
                r < size - 1 ? (solvedBoard[r][c] > solvedBoard[r + 1][c] ? '∨' : '∧') : null;
                ...
                <div key={`${r}-${c}`} className="relative">
                  <button
                    ...
                    className={`
                         ...
                         ${isFixed ? 'bg-green-900/40 text-green-100 cursor-default' : 'bg-slate-800/30 text-green-300 hover:bg-slate-800/50'}
                         ${isSelected ? 'ring-inset ring-4 ring-green-400 bg-green-600/50 shadow-[0_0_15px_rgba(34,197,94,0.5)] z-20' : ''}
                       `}
                  >
                    {cell !== 0 ? cell : ''}
                  </button>
                  ...
                  {RightRelation && c !== size - 1 && (c + 1) % boxCols !== 0 && (
                    <div className="absolute top-1/2 -right-2 transform -translate-y-1/2 text-green-500 font-bold z-30 pointer-events-none bg-slate-900 rounded-full w-4 h-4 flex items-center justify-center text-xs border border-green-800">
                      {RightRelation}
                    </div>
                  )}
                  {BottomRelation && r !== size - 1 && (r + 1) % boxRows !== 0 && (
                    <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 text-green-500 font-bold z-30 pointer-events-none bg-slate-900 rounded-full w-4 h-4 flex items-center justify-center text-xs border border-green-800">
                      {BottomRelation}
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-green-600 to-green-800 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-green-400/50"
            >
              {num}
            </button>
          ))}
...
export default GreaterThanSudokuGame;
