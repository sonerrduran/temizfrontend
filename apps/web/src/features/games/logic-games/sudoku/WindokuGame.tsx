...
const WindokuGame: React.FC<Props> = ({ difficulty, grade, onComplete, onExit }) => {
  ...
  // Windoku has 4 windows, structurally identical to Hyper Sudoku regions
  const windowRegions = [
    [1, 1, 3, 3],
    [1, 5, 3, 7],
    [5, 1, 7, 3],
  ...
  const isWindow = (r: number, c: number) => {
    for (let box of windowRegions) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 flex flex-col items-center">
      <div className="w-full flex justify-between items-center mb-6">
        ...
        <h2 className="text-3xl font-black text-white">Windoku</h2>
              ...
              return (
                <button
                  ...
                  className={`
                             ...
                             ${borderClasses} 
                             ${inWin ? 'bg-sky-500/30' : 'bg-slate-800'}
                             ${isSelected ? 'ring-4 ring-yellow-400 z-10 scale-110 shadow-xl' : ''}
                             ${isInitial ? 'text-white' : 'text-sky-300'}
        ...
        <div className="grid grid-cols-5 md:grid-cols-9 gap-2">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button
              ...
              className="bg-sky-500/20 hover:bg-sky-500/40 disabled:opacity-50 text-sky-100 font-bold py-3 md:py-4 rounded-xl text-xl shadow-lg transition-all active:scale-95 border border-sky-500/30"
            >
              {num}
            </button>
          ))}
