...
interface LittleKillerSudokuGameProps {
  grade: number;
...
const LittleKillerSudokuGame: React.FC<LittleKillerSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
    ...
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-red-500/30 shadow-2xl relative overflow-hidden bounce-in">
      ...
      <div className="flex flex-col items-center">
        <div className="relative mb-8 p-6">
          {/* Diagonal hints wrapping the board */}
          <div className="absolute top-0 left-0 text-red-400 font-bold text-sm transform -rotate-45">
            ↘ 15
          </div>
          <div className="absolute top-0 right-0 text-red-400 font-bold text-sm transform rotate-45">
            12 ↙
          </div>
          <div className="absolute bottom-0 left-0 text-red-400 font-bold text-sm transform rotate-45">
            ↗ 20
          </div>
          <div className="absolute bottom-0 right-0 text-red-400 font-bold text-sm transform -rotate-45">
            18 ↖
          </div>

          <div
            className="bg-red-950/50 p-2 md:p-3 rounded-xl border-4 border-red-900/50 shadow-[0_0_20px_rgba(239,68,68,0.1)] grid gap-0.5 relative z-10"
            style={{
              ...
              row.map((cell, c) => {
                ...
                const borderRight =
                  (c + 1) % boxCols === 0 && c !== size - 1
                    ? 'border-r-4 border-red-700/80'
                    : 'border-r border-red-900/50';
                const borderBottom =
                  (r + 1) % boxRows === 0 && r !== size - 1
                    ? 'border-b-4 border-red-700/80'
                ...
                return (
                  <button
                    ...
                    className={`
                         ...
                         ${isFixed ? 'bg-red-900/40 text-red-100 cursor-default' : 'bg-slate-800/30 text-red-300 hover:bg-slate-800/50'}
                         ${isSelected ? 'ring-inset ring-4 ring-red-400 bg-red-600/50 shadow-[0_0_15px_rgba(239,68,68,0.5)] z-20' : ''}
                       `}
                  >
                    {cell !== 0 ? cell : ''}
                  </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-red-600 to-red-800 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-red-400/50"
            >
              {num}
            </button>
          ))}
...
export default LittleKillerSudokuGame;
