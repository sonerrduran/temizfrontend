...
interface LocationPuzzleGameProps {
  onBack: () => void;
}
...
interface Item {
  ...
  emoji: string;
  name: string;
  correctPosition: 'top' | 'bottom' | 'left' | 'right' | 'center';
  currentPosition: string | null;
}

const LocationPuzzleGame: React.FC<LocationPuzzleGameProps> = ({ onBack }) => {
  ...
  const allItems = [
    { emoji: '☀️', name: 'Güneş', position: 'top' as const },
    { emoji: '🌙', name: 'Ay', position: 'top' as const },
    { emoji: '⭐', name: 'Yıldız', position: 'top' as const },
    { emoji: '🌳', name: 'Ağaç', position: 'bottom' as const },
    { emoji: '🌻', name: 'Çiçek', position: 'bottom' as const },
    { emoji: '🦋', name: 'Kelebek', position: 'center' as const },
    { emoji: '🐦', name: 'Kuş', position: 'top' as const },
    { emoji: '🐛', name: 'Tırtıl', position: 'bottom' as const },
  ];
  ...
  const handlePositionClick = (position: 'top' | 'bottom' | 'left' | 'right' | 'center') => {
    if (selectedItem === null) return;

    const item = items.find((i) => i.id === selectedItem);
    if (!item || item.currentPosition !== null) return;

    if (item.correctPosition === position) {
      const newItems = items.map((i) =>
        i.id === selectedItem ? { ...i, currentPosition: position } : i
      );
      setItems(newItems);
      setScore(score + 10);
      setFeedback('✅ Doğru konum!');
      setSelectedItem(null);

      if (newItems.every((i) => i.currentPosition !== null)) {
        setTimeout(() => {
          if (level < 5) {
            setLevel(level + 1);
          } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-white text-4xl md:text-5xl font-black">🎯 Konum Bulmaca</h1>
          <p className="text-slate-400 text-lg mt-2">Nesnelerin doğru konumunu bul!</p>
        </div>
          ...
          <div className="bg-gradient-to-br from-cyan-500 via-blue-500 to-cyan-600 rounded-2xl p-8 md:p-12 mb-8">
            ...
            >
              ...
              <button
                ...
                className="bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-2xl p-6 min-h-[100px] flex flex-col items-center justify-center"
              >
                <span className="text-3xl mb-2">⬆️</span>
                <span className="text-white font-bold">YUKARI</span>
                <div className="flex gap-2 mt-2">
                  {items
                    .filter((i) => i.currentPosition === 'top')
                    .map((item) => (
              ...
              <button
                ...
                className="bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-2xl p-6 flex flex-col items-center justify-center"
              >
                <span className="text-3xl mb-2">⬅️</span>
                <span className="text-white font-bold">SOL</span>
                <div className="flex gap-2 mt-2">
                  {items
                    .filter((i) => i.currentPosition === 'left')
                    .map((item) => (
                      <span key={item.id} className="text-3xl">
                        {item.emoji}
                      </span>
              ...
              <button
                onClick={() => handlePositionClick('center')}
                className="bg-cyan-700/40 hover:bg-cyan-600/40 border-2 border-cyan-300 rounded-2xl p-6 flex flex-col items-center justify-center"
              >
                <span className="text-3xl mb-2">🎯</span>
                <span className="text-white font-bold">ORTA</span>
                <div className="flex gap-2 mt-2">
                  {items
                    .filter((i) => i.currentPosition === 'center')
                    .map((item) => (
                      <span key={item.id} className="text-3xl">
                        {item.emoji}
                      </span>
              ...
              >
                <span className="text-3xl mb-2">➡️</span>
                <span className="text-white font-bold">SAĞ</span>
                <div className="flex gap-2 mt-2">
                  {items
                    .filter((i) => i.currentPosition === 'right')
                    .map((item) => (
                      <span key={item.id} className="text-3xl">
                        {item.emoji}
                      </span>
              ...
              >
                <span className="text-3xl mb-2">⬇️</span>
                <span className="text-white font-bold">AŞAĞI</span>
                <div className="flex gap-2 mt-2">
                  {items
                    .filter((i) => i.currentPosition === 'bottom')
                    .map((item) => (
                      <span key={item.id} className="text-3xl">
                        {item.emoji}
                      </span>
            ...
            <div className="bg-cyan-700/40 rounded-2xl p-6 border-2 border-cyan-300">
              <h3 className="text-xl font-black text-white text-center mb-4">Nesneleri Seç</h3>
              <div className="flex flex-wrap gap-4 justify-center">
                {items
                  .filter((i) => i.currentPosition === null)
                  .map((item) => (
                    <button
                      key={item.id}
                      onClick={() => handleItemClick(item.id)}
                      className={`p-4 rounded-2xl text-5xl transition-all transform ${
                        selectedItem === item.id
                          ? 'bg-yellow-400 scale-110 border-4 border-yellow-300'
                          : 'bg-cyan-800/40 hover:bg-cyan-700/40 border-2 border-cyan-300 hover:scale-105'
                      }`}
                    >
                      {item.emoji}
                    </button>
          ...
          <div className="bg-slate-800/90 p-6 md:p-8 rounded-3xl border border-cyan-500/30 max-w-md w-full">
            ...
            <ul className="text-white/90 text-left space-y-3 mb-8 text-sm md:text-base">
              <li className="flex gap-2">
                <span className="text-cyan-400 font-bold">1.</span> Nesneleri doğru konuma yerleştir
              </li>
              <li className="flex gap-2">
                <span className="text-cyan-400 font-bold">2.</span> Önce nesneyi seç, sonra konuma
                tıkla
              </li>
              <li className="flex gap-2">
                <span className="text-cyan-400 font-bold">3.</span> Güneş yukarıda, ağaç aşağıda
                olmalı
              </li>
              <li className="flex gap-2">
                <span className="text-cyan-400 font-bold">4.</span> Tüm nesneleri yerleştir ve
                seviyeyi tamamla!
              </li>
            </ul>
            <button
...
export default LocationPuzzleGame;
