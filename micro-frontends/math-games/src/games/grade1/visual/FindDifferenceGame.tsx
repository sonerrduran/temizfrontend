...
const FindDifferenceGame: React.FC<FindDifferenceGameProps> = ({ onComplete, onExit }) => {
  ...
  const handleAnswer = (index: number) => {
    ...
    const correct = index === patterns[currentIndex].different;
    if (correct) {
      setScore(score + 10);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-600 via-blue-500 to-indigo-500 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          <h3 className="text-3xl font-black text-blue-600 mb-8 text-center">Farklı Olanı Bul!</h3>
          ...
          <div className="grid grid-cols-5 gap-4 mb-8">
            {current.items.map((item, index) => (
              <button
                ...
                className={`aspect-square rounded-2xl font-black text-6xl transition-all flex items-center justify-center ${
                  showFeedback
                    ? index === current.different
                      ? 'bg-green-500 text-white scale-110 animate-bounce'
                      : index === selectedAnswer
                        ? 'bg-red-500 text-white'
                        : 'bg-gray-200 text-gray-400'
                    : 'bg-gradient-to-br from-yellow-400 to-orange-500 hover:scale-110 active:scale-95'
                }`}
              >
                {item}
              </button>
          ...
          {showFeedback && (
            <div
              className={`p-4 rounded-2xl text-center font-bold text-xl ${
                selectedAnswer === current.different
                  ? 'bg-green-100 text-green-700'
                  : 'bg-red-100 text-red-700'
              }`}
            >
              {selectedAnswer === current.different
                ? '🎉 Harika! Farklı olanı buldun!'
                : `❌ Yanlış! Farklı olan ${current.different + 1}. sıradaydı!`}
            </div>
          )}
