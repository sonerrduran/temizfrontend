...
interface SubtractionBasketGameProps {
  onExit: () => void;
}

const SubtractionBasketGame: React.FC<SubtractionBasketGameProps> = ({ onExit }) => {
  const [totalBalls, setTotalBalls] = useState(0);
  const [thrownBalls, setThrownBalls] = useState(0);
  ...
  const generateQuestion = () => {
    const total = Math.floor(Math.random() * 6) + 4; // 4-9 balls
    const thrown = Math.floor(Math.random() * (total - 1)) + 1;
    setTotalBalls(total);
    setThrownBalls(thrown);
    setUserAnswer('');
    setFeedback('');
  };

  const checkAnswer = () => {
    const correctAnswer = totalBalls - thrownBalls;
          ...
          <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-orange-600 rounded-2xl p-8 md:p-12 mb-8">
            <div className="flex flex-col items-center gap-6">
              <div className="bg-orange-700/40 p-6 rounded-2xl border-2 border-orange-400 w-full max-w-md">
                <p className="text-white text-lg mb-3 text-center">{totalBalls} top vardı:</p>
              ...
              <div className="text-center">
                <p className="text-white text-2xl font-black mb-4">
                  {thrownBalls} topu potaya attın!
                </p>
                <p className="text-white text-xl font-bold">Kaç top kaldı?</p>
              </div>
...
export default SubtractionBasketGame;
