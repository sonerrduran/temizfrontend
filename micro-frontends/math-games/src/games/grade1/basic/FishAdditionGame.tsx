...
interface FishAdditionGameProps {
  onExit: () => void;
}

const FishAdditionGame: React.FC<FishAdditionGameProps> = ({ onExit }) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          {/* Question Card - Cyan Gradient (Oyun Kutusu Rengi) */}
          <div className="bg-gradient-to-br from-cyan-500 via-teal-500 to-cyan-600 rounded-2xl p-8 md:p-12 mb-8">
            <div className="flex items-center justify-center gap-3 md:gap-4 flex-wrap">
              {/* First Fish */}
              <div className="flex flex-col items-center bg-cyan-700/40 p-4 md:p-5 rounded-2xl border-2 border-cyan-400 w-[140px] md:w-[160px]">
          ...
          {/* Check Button - Açık Cyan (Net Beyaz Yazı, Gölgesiz) */}
          <button
            onClick={checkAnswer}
            disabled={!userAnswer || showFeedback}
            className="w-full bg-cyan-500 hover:bg-cyan-400 text-white text-2xl md:text-3xl font-black py-5 md:py-6 rounded-2xl transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
          >
            Kontrol Et
          </button>
...
export default FishAdditionGame;
