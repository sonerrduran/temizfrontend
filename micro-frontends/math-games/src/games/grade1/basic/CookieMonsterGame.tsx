...
interface CookieMonsterGameProps {
  onExit: () => void;
}

const CookieMonsterGame: React.FC<CookieMonsterGameProps> = ({ onExit }) => {
  const [totalCookies, setTotalCookies] = useState(0);
  const [eatenCookies, setEatenCookies] = useState(0);
  ...
  const generateQuestion = () => {
    const total = Math.floor(Math.random() * 5) + 3; // 3-7 cookies
    const eaten = Math.floor(Math.random() * (total - 1)) + 1; // at least 1, max total-1
    setTotalCookies(total);
    setEatenCookies(eaten);
    setUserAnswer('');
    setFeedback('');
  };

  const checkAnswer = () => {
    const correctAnswer = totalCookies - eatenCookies;
    ...
    if (isCorrect) {
      ...
      setFeedback('✅ Doğru! Harika sayma!');
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          {/* Question Card - Orange Gradient (Oyun Kutusu Rengi) */}
          <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-orange-600 rounded-2xl p-8 md:p-12 mb-8">
            {/* Story */}
            <div className="text-center mb-8">
              <div className="text-7xl mb-4">👹</div>
              <p className="text-white text-xl mb-6">Canavar kurabiye yedi!</p>
            </div>

            {/* Visual Representation */}
            <div className="flex flex-col items-center gap-6">
              {/* Total Cookies */}
              <div className="bg-orange-700/40 p-6 rounded-2xl border-2 border-orange-400 w-full max-w-md">
                <p className="text-white text-lg mb-3 text-center">
                  Başta {totalCookies} kurabiye vardı:
                </p>
                <div className="flex gap-2 flex-wrap justify-center">
                  {Array.from({ length: totalCookies }).map((_, i) => (
              ...
              <div className="text-center">
                <p className="text-white text-2xl font-black mb-4">
                  Canavar {eatenCookies} kurabiye yedi!
                </p>
                <p className="text-white text-xl font-bold">Kaç kurabiye kaldı?</p>
              </div>
          ...
          {/* Check Button - Açık Orange (Net Beyaz Yazı, Gölgesiz) */}
          <button
            onClick={checkAnswer}
            disabled={!userAnswer || showFeedback}
            className="w-full bg-orange-500 hover:bg-orange-400 text-white text-2xl md:text-3xl font-black py-5 md:py-6 rounded-2xl transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
          >
            Kontrol Et
          </button>
...
export default CookieMonsterGame;
