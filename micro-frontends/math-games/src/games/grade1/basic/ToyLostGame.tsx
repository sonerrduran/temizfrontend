...
interface ToyLostGameProps {
  onExit: () => void;
}

const ToyLostGame: React.FC<ToyLostGameProps> = ({ onExit }) => {
  const [totalToys, setTotalToys] = useState(0);
  const [lostToys, setLostToys] = useState(0);
  ...
  const generateQuestion = () => {
    const total = Math.floor(Math.random() * 6) + 4; // 4-9 toys
  ...
  const checkAnswer = () => {
    const correctAnswer = totalToys - lostToys;
        ...
        <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-700 p-8">
          ...
          <div className="bg-gradient-to-br from-pink-500 via-rose-500 to-pink-600 rounded-2xl p-8 md:p-12 mb-8">
            ...
            <div className="flex flex-col items-center gap-6">
              {/* Toys */}
              <div className="bg-pink-700/40 p-6 rounded-2xl border-2 border-pink-400 w-full max-w-md">
                <p className="text-white text-lg mb-3 text-center">{totalToys} oyuncak vardı:</p>
                <div className="flex gap-2 flex-wrap justify-center">
                  {Array.from({ length: totalToys }).map((_, i) => (
              ...
              <div className="text-center">
                <p className="text-white text-2xl font-black mb-4">{lostToys} oyuncak kayboldu!</p>
                <p className="text-white text-xl font-bold">Kaç oyuncak kaldı?</p>
              </div>
          ...
          {/* Check Button - Açık Pink (Net Beyaz Yazı, Gölgesiz) */}
          <button
            onClick={checkAnswer}
            disabled={!userAnswer || showFeedback}
            className="w-full bg-pink-500 hover:bg-pink-400 text-white text-2xl md:text-3xl font-black py-5 md:py-6 rounded-2xl transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
          >
            Kontrol Et
          </button>
