...
interface DivisionRaceGameProps {
  onBack: () => void;
}

const DivisionRaceGame: React.FC<DivisionRaceGameProps> = ({ onBack }) => {
  ...
  const generateQuestion = () => {
    const divisorNum = Math.floor(Math.random() * 8) + 2;
    const quotient = Math.floor(Math.random() * 9) + 2;
    const dividendNum = divisorNum * quotient;

    setDividend(dividendNum);
    setDivisor(divisorNum);

    const wrongOptions = [quotient + 1, quotient - 1, quotient + 2].filter((v) => v > 0);

    const allOptions = [quotient, ...wrongOptions.slice(0, 2)].sort(() => Math.random() - 0.5);
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-600 via-rose-500 to-red-500 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          <h3 className="text-3xl font-black text-pink-600 mb-8 text-center">Bölme Yarışı 🏁</h3>

          <div className="bg-gradient-to-br from-purple-100 to-pink-100 rounded-2xl p-12 mb-8 border-4 border-purple-400">
            <div className="flex items-center justify-center gap-8 text-8xl font-black text-pink-600">
              <span>{dividend}</span>
