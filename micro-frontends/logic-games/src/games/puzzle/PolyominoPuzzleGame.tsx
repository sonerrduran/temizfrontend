...
interface PolyominoPuzzleGameProps {
  onBack: () => void;
}

const PolyominoPuzzleGame: React.FC<PolyominoPuzzleGameProps> = ({ onBack }) => {
  ...
  // Polyomino shapes (various sizes)
  const polyominoes: { [key: number]: { shape: number[][]; color: string; name: string } } = {
    1: { shape: [[1, 1, 1]], color: 'bg-red-500', name: 'I3' },
    2: {
      shape: [
        [1, 1],
        [1, 0],
    ...
    3: { shape: [[1, 1, 1, 1]], color: 'bg-green-500', name: 'I4' },
    4: {
      shape: [
        [1, 1],
        [1, 1],
    ...
    8: { shape: [[1, 1, 1, 1, 1]], color: 'bg-cyan-500', name: 'I5' },
  };
  ...
  const canPlacePiece = (shape: number[][], row: number, col: number): boolean => {
    for (let i = 0; i < shape.length; i++) {
      for (let j = 0; j < shape[i].length; j++) {
        if (shape[i][j] === 1) {
          const newRow = row + i;
          const newCol = col + j;
          if (newRow >= 8 || newCol >= 12 || newRow < 0 || newCol < 0) return false;
  ...
  const placePiece = (pieceId: number, row: number, col: number) => {
    ...
    const shape = polyominoes[pieceId].shape;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-900 via-fuchsia-900 to-pink-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-white">Polyomino Bulmacası</h1>
        ...
        <div className="flex gap-4 mb-6">
          ...
          <div className="flex-1 bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            ...
            <div className="text-2xl font-bold text-white">{placedPieces.size}/8</div>
          </div>
          <div className="flex-1 bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <ul className="text-white/90 text-sm space-y-1">
                  <li>• Polyomino parçalarını tahtaya yerleştirin</li>
                  <li>• Her parça farklı sayıda kareden oluşur</li>
                  <li>• Bir parça seçin, sonra tahtaya tıklayın</li>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div key={i} className="flex">
                  {row.map((cell, j) => (
                    <button
                      ...
                      className={`w-8 h-8 md:w-10 md:h-10 border border-gray-600 transition-all ${
                        cell === 0 ? 'bg-white/20 hover:bg-white/30' : polyominoes[cell].color
                      }`}
                    >
                      {cell !== 0 && (
                        <span className="text-white font-bold text-xs">
                          {polyominoes[cell].name}
                        </span>
                      )}
                    </button>
                  ))}
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(polyominoes).map(([id, piece]) => {
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
              ...
              <p className="text-white/90 mb-6">
                Polyomino Bulmacası'nı {formatTime(time)} sürede, {mistakes} hata ile tamamladınız!
              </p>
...
export default PolyominoPuzzleGame;
