...
interface QuadSudokuGameProps {
  onBack: () => void;
}

const QuadSudokuGame: React.FC<QuadSudokuGameProps> = ({ onBack }) => {
  ...
  const generatePuzzle = () => {
    // Generate 4 overlapping 9x9 Sudoku grids in 2x2 layout
    ...
    for (let g = 0; g < 4; g++) {
      ...
      for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
          solution[i][j] = ((i * 3 + Math.floor(i / 3) + j + g * 3) % 9) + 1;
        }
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-white">Dörtlü Sudoku</h1>
                  ...
                  <div key={i} className="flex">
                    {row.map((cell, j) => (
                      <button
                        ...
                        className={`w-7 h-7 md:w-8 md:h-8 border font-bold text-xs md:text-sm transition-all ${
        ...
        {isComplete && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
              ...
              <h2 className="text-3xl font-black text-white mb-4">Muhteşem!</h2>
              <p className="text-white/90 mb-6">
                Dörtlü Sudoku'yu {formatTime(time)} sürede, {mistakes} hata ile tamamladınız!
              </p>
