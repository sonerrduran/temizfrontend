...
const generateSolvedBoard = (size: number): number[][] => {
  // Sudoku X requires both main diagonals to contain 1-size
...
const SudokuXGame: React.FC<SudokuXGameProps> = ({ grade, difficulty, onComplete, onExit }) => {
  ...
  const initGame = useCallback(() => {
    ...
    if (solved[0][0] === 0) {
      // Fallback generated without X constraint if it fails
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-fuchsia-500/30 shadow-2xl relative overflow-hidden bounce-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-center flex-1">
          <h2 className="text-2xl md:text-3xl font-black italic text-fuchsia-400 drop-shadow-lg uppercase tracking-tight">
            Sudoku X
          </h2>
          <div className="text-xs font-bold text-fuchsia-200 mt-1 uppercase tracking-widest">
            {size}x{size} Izgara | (Temel Sürüm) | Hata: {mistakes}/{maxMistakes}
          </div>
        </div>
        <div
          className={`w-16 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 ${timeLeft <= 30 ? 'border-red-500 text-red-500 animate-pulse' : 'border-fuchsia-400 text-fuchsia-400'}`}
        >
          {formatTime(timeLeft)}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-fuchsia-950/50 p-2 md:p-3 rounded-xl border-4 border-fuchsia-900/50 shadow-[0_0_20px_rgba(217,70,239,0.1)] grid gap-0.5 mb-8 relative"
          style={{
            gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))`,
            width: size === 4 ? '280px' : '360px',
          }}
        >
          {/* Visual X behind the grid */}
          <div className="absolute inset-0 pointer-events-none opacity-20 overflow-hidden rounded-xl">
            <div
              className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-fuchsia-500 to-transparent transform scale-[1.5] -rotate-45"
              style={{ clipPath: 'polygon(48% 0, 52% 0, 52% 100%, 48% 100%)' }}
            ></div>
            <div
              className="absolute top-0 right-0 w-full h-full bg-gradient-to-bl from-fuchsia-500 to-transparent transform scale-[1.5] rotate-45"
              style={{ clipPath: 'polygon(48% 0, 52% 0, 52% 100%, 48% 100%)' }}
            ></div>
          </div>
            ...
            row.map((cell, c) => {
              ...
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1
                  ? 'border-r-4 border-fuchsia-700/80'
                  : 'border-r border-fuchsia-900/50';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1
                  ? 'border-b-4 border-fuchsia-700/80'
                  : 'border-b border-fuchsia-900/50';

              const isDiagonal = r === c || r + c === size - 1;
              ...
              return (
                <button
                  ...
                  className={`
                     aspect-square flex items-center justify-center text-3xl font-black transition-all relative z-10
                     ${borderRight} ${borderBottom}
                     ${isFixed ? (isDiagonal ? 'bg-fuchsia-900/70 text-fuchsia-100' : 'bg-fuchsia-900/40 text-fuchsia-100') : isDiagonal ? 'bg-fuchsia-800/40 text-fuchsia-300' : 'bg-slate-800/30 text-fuchsia-300'} hover:bg-slate-800/50
                     ${isSelected ? 'ring-inset ring-4 ring-fuchsia-400 bg-fuchsia-600/50 shadow-[0_0_15px_rgba(217,70,239,0.5)] z-20' : ''}
                   `}
                >
                  {cell !== 0 ? cell : ''}
                </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-fuchsia-600 to-purple-800 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-fuchsia-400/50"
            >
              {num}
            </button>
          ))}
