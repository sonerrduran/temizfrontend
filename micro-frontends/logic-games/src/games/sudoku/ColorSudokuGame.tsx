...
interface ColorSudokuGameProps {
  grade: number;
...
const ColorSudokuGame: React.FC<ColorSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
  ...
  // Fixed colors mapping for specific digits
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-sky-400/30 shadow-2xl relative overflow-hidden bounce-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-center flex-1">
          <h2 className="text-2xl md:text-3xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-green-400 to-blue-400 drop-shadow-lg uppercase tracking-tight">
            Renkli Sudoku
          </h2>
      ...
      <div className="flex flex-col items-center">
        <div
          className="bg-slate-950/50 p-2 md:p-3 rounded-xl border-4 border-slate-700/50 shadow-inner grid gap-1 mb-8"
          style={{
        ...
        >
          {board.map((row, r) =>
            row.map((cell, c) => {
              ...
              const actualVal = cell !== 0 ? cell : solvedBoard[r][c];
              const cellColorBg =
                isFixed || cell !== 0
                  ? colorMap[cell]
                  : isSelected
              ...
              const boxCols = size === 4 ? 2 : 3;
              const borderRight =
                (c + 1) % boxCols === 0 && c !== size - 1 ? 'border-r-4 border-slate-600' : '';
              const borderBottom =
                (r + 1) % boxRows === 0 && r !== size - 1 ? 'border-b-4 border-slate-600' : '';
              ...
              return (
                <button
                  ...
                  className={`
                     ...
                     ${cellColorBg}
                     ${isFixed ? 'text-white cursor-default' : 'text-slate-300 hover:brightness-125'}
                     ${isSelected ? 'ring-4 ring-sky-400 bg-sky-600/50 shadow-[0_0_15px_rgba(56,189,248,0.5)] z-10' : ''}
                   `}
                >
                  {cell !== 0 ? cell : ''}
                </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className={`w-14 h-14 md:w-16 md:h-16 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-white/20 ${colorMap[num]}`}
            >
              {num}
            </button>
          ))}
...
export default ColorSudokuGame;
