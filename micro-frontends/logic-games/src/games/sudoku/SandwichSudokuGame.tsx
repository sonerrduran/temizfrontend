...
interface SandwichSudokuGameProps {
  grade: number;
...
const SandwichSudokuGame: React.FC<SandwichSudokuGameProps> = ({
  grade,
  difficulty,
  onComplete,
  onExit,
    ...
    <div className="w-full max-w-2xl mx-auto bg-slate-900/90 backdrop-blur-3xl rounded-[40px] p-6 md:p-8 border border-yellow-500/30 shadow-2xl relative overflow-hidden bounce-in">
      ...
      <div className="flex flex-col items-center">
        <div className="flex relative mb-8">
          {/* Row clues */}
          <div className="flex flex-col justify-around mr-2 mt-4 text-yellow-500 font-bold opacity-70">
          ...
          <div>
            {/* Col clues */}
            <div className="flex justify-around mb-2 text-yellow-500 font-bold opacity-70">
              {Array.from({ length: size }).map((_, i) => (
                <div key={i}>{8 + i}</div>
              ))}
            </div>
            <div
              className="bg-yellow-950/50 p-2 md:p-3 rounded-xl border-4 border-yellow-900/50 shadow-inner grid gap-0.5 relative z-10"
              style={{
                  ...
                  return (
                    <button
                      ...
                      className={`
                             ...
                             ${isFixed ? 'bg-yellow-900/40 text-yellow-100 cursor-default' : 'bg-slate-800/30 text-yellow-300 hover:bg-slate-800/50'}
                             ${isSelected ? 'ring-inset ring-4 ring-yellow-400 bg-yellow-600/50 shadow-[0_0_15px_rgba(250,204,21,0.5)] z-20' : ''}
                           `}
                    >
                      {cell !== 0 ? cell : ''}
                    </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-sm">
          {Array.from({ length: size }, (_, i) => i + 1).map((num) => (
            <button
              ...
              className={`w-14 h-14 md:w-16 md:h-16 text-white rounded-2xl font-black text-2xl md:text-3xl shadow-lg hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed transition-transform border border-yellow-400/50 ${num === 1 || num === size ? 'bg-gradient-to-br from-orange-500 to-red-600' : 'bg-gradient-to-br from-yellow-500 to-amber-600'}`}
            >
              {num}
            </button>
          ))}
...
export default SandwichSudokuGame;
